...
async function signInWithPopup$1(auth2, provider2, resolver) {
  const authInternal = _castAuth(auth2);
...
var wa = x$1("Opera"), y = x$1("Trident") || x$1("MSIE"), xa = x$1("Edge"), ya = xa || y, za$1 = x$1("Gecko") && !(-1 != sa().toLowerCase().indexOf("webkit") && !x$1("Edge")) && !(x$1("Trident") || x$1("MSIE")) && !x$1("Edge"), Aa$1 = -1 != sa().toLowerCase().indexOf("webkit") && !x$1("Edge");
function Ba() {
  var a = l.document;
      ...
      a: {
        ...
        if (!(3 > r) && (3 != r || ya || this.g && (this.h.h || this.g.fa() || hc$1(this.g)))) {
          ...
          if (this.i) {
            ...
            this.P ? (lc$1(this, r, h), ya && this.i && 3 == r && (Eb(this.S, this.T, "tick", this.hb), this.T.start())) : (E(this.j, this.m, h, null), kc(this, h));
            4 == r && P$1(this);
...
function Dc(a, b2) {
  if (a.forEach && "function" == typeof a.forEach)
    ...
    for (var c = Cc(a), d = Bc$1(a), e = d.length, f = 0; f < e; f++)
...
function T(a, b2) {
  ...
  if (a instanceof T) {
    ...
    a && (c = String(a).match(Ec$1)) ? (this.h = !!b2, Gc$1(this, c[1] || "", true), this.s = Kc(c[2] || ""), this.g = Kc(c[3] || "", true), Hc(this, c[4]), this.l = Kc(c[5] || "", true), Jc(this, c[6] || "", true), this.o = Kc(c[7] || "")) : (this.h = !!b2, this.i = new Ic(null, this.h));
}
T.prototype.toString = function() {
  var a = [], b2 = this.j;
  b2 && a.push(Lc(b2, Mc$1, true), ":");
  var c = this.g;
  if (c || "file" == b2)
    a.push("//"), (b2 = this.s) && a.push(Lc(b2, Mc$1, true), "@"), a.push(encodeURIComponent(String(c)).replace(/%25([0-9a-fA-F]{2})/g, "%$1")), c = this.m, null != c && a.push(":", String(c));
  if (c = this.l)
    this.g && "/" != c.charAt(0) && a.push("/"), a.push(Lc(c, "/" == c.charAt(0) ? Nc$1 : Oc, true));
...
function Jc(a, b2, c) {
  b2 instanceof Ic ? (a.i = b2, Qc$1(a.i, a.h)) : (c || (b2 = Lc(b2, Rc)), a.i = new Ic(b2, a.h));
}
function S$1(a, b2, c) {
  a.i.set(b2, c);
}
...
var Mc$1 = /[#\/\?@]/g, Oc = /[#\?:]/g, Nc$1 = /[#\?]/g, Rc = /[#\?@]/g, Pc = /#/g;
function Ic(a, b2) {
  this.h = this.g = null;
  this.i = a || null;
  this.j = !!b2;
...
function R(a, b2) {
  ...
  if (2 == b2) {
    ...
    a.l && (c = null);
    var d = q$1(a.kb, a);
    c || (c = new T("//www.google.com/images/cleardot.gif"), l.location && "http" == l.location.protocol || Gc$1(c, "https"), dc(c));
    cd(c.toString(), d);
  } else
    I(2);
  a.G = 0;
...
const db = wh(app);
const signInWithEmailAndPassword = (email, password) => signInWithEmailAndPassword$1(
  auth,
  email,
  password
);
const provider = new GoogleAuthProvider();
const signInWithPopup = () => signInWithPopup$1(
  auth,
  provider
);
const createUserWithEmailAndPassword = (email, password) => createUserWithEmailAndPassword$1(
  auth,
  email,
  password
);
...
function login(navigateTo2) {
  ...
  titleLogin.textContent = "Iniciar Sesi\xF3n";
  titleLogin.id = "title";
  ...
  textPass.textContent = "Contrase\xF1a";
  ...
  buttonLogin.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (error2.code === "auth/user-not-found") {
        ...
        textError.innerHTML = "Contrase\xF1a equivocada";
      } else if (error2.code === "auth/missing-password") {
        textError.innerHTML = "Ingrese su contrase\xF1a";
      } else if (error2.code === "auth/invalid-email") {
        textError.innerHTML = "Usuario no v\xE1lido";
      } else if (error2.code === "auth/internal-error") {
        textError.innerHTML = "Error interno, intentelo de nuevo";
      }
      console.log("error", error2.code);
      console.log("holiwi", error2.message);
    }
  });
  buttonGoogle.addEventListener("click", async () => {
    try {
      ...
      console.log("google error", error2);
    }
  });
...
function register(navigateTo2) {
  ...
  textPassConfirm.textContent = "Confirmar contrase\xF1a";
    ...
    if (inputPassConfirm.value === inputPass.value) {
      try {
        ...
        if (error2.code === "auth/email-already-in-use") {
          textError.innerHTML = "El correo ingresado ya est\xE1 en uso";
        } else if (error2.code === "auth/invalid-email") {
          textError.innerHTML = "Correo no v\xE1lido";
        } else if (error2.code === "auth/weak-password") {
          textError.innerHTML = "La contrase\xF1a debe tener al menos 6 car\xE1cteres";
        } else if (error2.code === "auth/missing-password") {
          textError.innerHTML = "Debe ingresar una contrase\xF1a";
        } else if (error2.code === "auth/missing-email") {
          textError.innerHTML = "Debe ingresar un correo";
        } else if (error2.code) {
          textError.innerHTML = "Oops, algo sali\xF3 mal";
          console.log("error", error2.code);
          console.log("holiwi", error2.message);
        }
      }
    } else if (inputPassConfirm.value === "") {
      textError.innerHTML = "Debe confirmar la contrase\xF1a";
    } else if (inputPassConfirm.value !== inputPass.value) {
      textError.innerHTML = "Las contrase\xF1as deben coincidir";
    }
  });
  return section;
}
...
function post() {
  ...
  btnLogOut.textContent = "Cerrar Sesi\xF3n";
...
function welcome(navigateTo2) {
  ...
  textWelcome.textContent = "Su cuenta ha sido creada con \xE9xito";
