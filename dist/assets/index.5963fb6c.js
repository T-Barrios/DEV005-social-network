    ...
    if (inputPassConfirm.value === inputPass.value) {
      try {
        ...
        if (error2.code === "auth/email-already-in-use") {
          ...
          textError.innerHTML = "La contrase\xF1a debe tener al menos 6 caracteres";
        } else if (error2.code === "auth/missing-password") {
...
function post() {
  ...
  textPost.placeholder = "\xBFHola, qu\xE9 hace?";
  ...
  textConfirm.textContent = "\xBFEst\xE1s seguro?";
  ...
  const editEvent = (buttonModalEdit, txtPost) => {
    ...
    btnEdit.addEventListener("click", async () => {
      const editRef = rh(db, "post", textEdit.dataset.id);
      await rf(editRef, {
  ...
  const deleteEvent = (txtPost) => {
    ...
    btnYes.addEventListener("click", async () => {
      await of(rh(db, "post", textEdit.dataset.id));
      containerModalEdit.style.display = "none";
  ...
  const likeEvent = (likesIcon, chayanne) => {
    ...
    likesIcon.addEventListener("click", async (e) => {
      if (!chayanne.includes(auth.currentUser.email)) {
        const likeRef = rh(db, "post", e.target.dataset.id);
        await rf(likeRef, {
          like: Sf(auth.currentUser.email)
        });
        ...
        const likeRef = rh(db, "post", e.target.dataset.id);
        await rf(likeRef, {
          like: Df(auth.currentUser.email)
        });
  ...
  const getPosts = (callback) => {
    const queryPost = dl(sh(db, "post"));
    cf(queryPost, (querySnapshot) => {
      const posts = [];
